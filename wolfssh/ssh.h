 ...
 * names what arrived; wolfSSH_get_error() names what the transport did, and a
 * flush on the same pass overwrites it. A loop that switches on
 * wolfSSH_get_error() to find WS_CHAN_RXD or WS_EXTDATA misses them whenever
 * that flush does not complete.
 * Back-pressure reaches the caller either as WS_WANT_WRITE in the return or
 * as WS_FATAL_ERROR with the want in wolfSSH_get_error(), so resolve a
 * WS_FATAL_ERROR through wolfSSH_get_error() rather than testing the return
 * for a want. Either way it is transient: call again. A caller that
 * tolerates only WS_WANT_READ drops live sessions, since a queued write now
 * reports WS_WANT_WRITE where it once reported WS_WANT_READ.
 * When the flush fails on the same pass:
 *   the return      keeps the event -- except that a clean receive reports
 *                   the flush's code, and WS_REKEYING is withheld
 *   get_error()     holds the flush's code -- except after a hard-failed
 *                   WS_CHANNEL_CLOSED, or a receive that failed for any
 *                   reason but want-read, where it keeps the receive's
